	...
	public function change_infos($post)
	{
		if($post['return_post_account_pass_change'] == 18041997)
		{
		    if(isset($post["password-new_1"]) && isset($post["password-new_2"]))
		    {
		    	...
		    	{
			    	$password = $this->user->check_post_sign_up_and_my_account($post['password-new_1']);
			    	$password_verification = $this->user->check_post_sign_up_and_my_account($post['password-new_2']);
