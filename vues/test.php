...
while($i < 70)
{
	$j = $i;
	$array_test_1[$i]["id"] = $j;
	$array_test_1[$i]["level"] = $j;
	$array_test_1[$i]["production"] = floor(((pow($j,1.6) * 42)) * Config::$rate_vg_prod);
	$array_test_1[$i]["prix"] = floor((pow($j,2.1) * 42));
	$array_test_1[$i]["time_construct"] = floor(((pow($j,2) * 42)) * 2);
	$i++;
}

affiche_pre($array_test_1);
...
while($i < 70)
{
	$j = $i;
	$array_test[$i]["id"] = $j;
	$array_test[$i]["level"] = $j;
	$array_test[$i]["production"] = floor(((pow($j,1.4) * 42)) * Config::$rate_pg_prod);
	$array_test[$i]["prix"] = floor((pow($j,2) * 42));
	$array_test[$i]["time_construct"] = floor(((pow($j,1.9) * 42)) * 2);
	$i++;
}
